...
// this shit could (and probably should) be implemented in a template.
// precisely zero (0) methods are unique to any sequence containers
...
void	PmergeMe::sort(std::deque<int>& X) {
	...
	std::size_t	upper_bound = s.size();
	std::size_t	lower_bound = 1; // rightmost element of s that has been inserted onto the main chain
	std::size_t	jacobsthal = 2; // lowest relevant N
	while (jacobsthalSequence(++jacobsthal) < upper_bound) {
